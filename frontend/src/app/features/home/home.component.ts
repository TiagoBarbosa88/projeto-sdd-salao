    ...
    <div class="space-y-6">
      ...
      <div class="space-y-4">
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2 md:items-stretch">
          <section class="flex flex-col rounded-2xl border border-slate-800 bg-slate-900 p-4">
            <header class="mb-3 border-b border-slate-800 pb-3">
              <h3 class="text-sm font-semibold text-white">Visualizacao</h3>
            </header>

            <div class="flex flex-1 flex-col gap-3">
              <div class="inline-flex w-fit rounded-xl border border-slate-700 bg-slate-950 p-1">
              ...
              <div class="flex flex-wrap gap-2">
                <button
                  type="button"
                  (click)="shiftPeriod(-1)"
                  class="rounded-lg border border-slate-700 px-3 py-1.5 text-sm text-slate-300 transition hover:border-violet-500 hover:text-white"
              ...
              <div
                class="mt-auto flex items-center justify-between gap-3 rounded-xl border border-slate-800 bg-slate-950/60 px-3 py-2"
              >
                <div class="min-w-0">
                  <p class="text-[10px] uppercase tracking-wider text-slate-500">Periodo</p>
                  <p class="truncate text-sm font-medium capitalize text-violet-300">{{ periodLabel() }}</p>
                </div>
                <div class="shrink-0 text-right">
                  <p class="text-lg font-semibold leading-none text-white">{{ periodSummary().inPeriod }}</p>
                  <p class="text-[10px] text-slate-500">
                    no periodo
                    @if (periodSummary().outOfPeriod > 0) {
                      <span class="text-amber-400/80"> · +{{ periodSummary().outOfPeriod }} fora</span>
                    }
                  </p>
                </div>
              </div>
          ...
          <section class="flex flex-col rounded-2xl border border-slate-800 bg-gradient-to-b from-slate-900 to-slate-950 p-4">
            <header class="mb-3 flex items-center justify-between gap-3 border-b border-slate-800 pb-3">
            ...
            <div class="grid grid-cols-7 gap-1 text-center text-[10px] font-medium uppercase tracking-wider text-slate-500">
              @for (label of weekdayLabels; track label) {
                <span class="py-0.5">{{ label }}</span>
              }
            </div>

            <div class="mt-1 grid flex-1 grid-cols-7 gap-1">
              @for (day of calendarDays(); track day.key) {
                <button
                  type="button"
                  (click)="selectCalendarDay(day.date)"
                  class="relative flex h-8 flex-col items-center justify-center rounded-lg text-xs transition md:h-9"
            ...
            <div class="mt-3 flex flex-wrap gap-x-3 gap-y-1 text-[10px] text-slate-400">
